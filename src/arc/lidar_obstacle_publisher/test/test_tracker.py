...
Run with `pytest` from the package root, or `colcon test --packages-select
...
def test_two_clusters_do_not_collapse_onto_one_previous_id():
    t = GreedyCentroidTracker()
    t.update([_cluster(0.0, 0.0, 1.0)], MAX_JUMP)      # id 0
    # both new clusters are within MAX_JUMP of the single old track, but a
    # previous id may only be claimed once -- the second cluster must get a
    # fresh id, not share id 0.
