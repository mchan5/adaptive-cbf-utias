# Extending Uncertainty-Calibrated CBF Gain Adaptation to Quadcopters
...
This repository integrates a **Probabilistic Ensemble Neural Network + Graph Attention Network (PENN+GAT)** model into a CBF-QP safety filter that adjusts its own gain online, based on the robot's state, nearby obstacles, and the model's epistemic unce

The flight path is the body-rate HOCBF-QP safety filter: C++/libtorch, running against PX4 firmware over the ROS 2 / DDS bridge, validated in Gazebo SITL and flown on hardware.
...
| Path | What it is |
|---|---|
| [`src/arc/`](src/arc) | My ROS 2 packages: the body-rate CBF-QP + PENN/GAT node (`single_vehicle_cbf_rate_arc`), obstacle perception (`lidar_obstacle_publisher`, `synthetic_obstacle_publisher`), the experiment GUI, and the HITL / SITL / hardware bench support. |
